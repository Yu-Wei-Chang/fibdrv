...
void *thread_func(void *arg)
{
    ...
    for (int i = 0; i <= offset; i++) {
        ...
        if ((sz = read(fd, &bn_buf, sizeof(bn_buf))) < 0) {
            printf("[%s] Reading failed at offset %d [%lld]\n",
                   args->thread_name, i, sz);
        } else {
            print_BigN_string(bn_buf, fib_str_buf, sizeof(fib_str_buf));
            printf("[%s] Reading from " FIB_DEV
                   " at offset %d, returned the sequence "
                   "%s.\n",
                   args->thread_name, i, fib_str_buf);
        }
    }
...
int main()
{
    ...
    pthread_t thread1;
    struct pthread_args thread_arg1;
    pthread_t thread2;
    struct pthread_args thread_arg2;
    pthread_t thread3;
    struct pthread_args thread_arg3;
    ...
    thread_arg1.fd = thread_arg2.fd = thread_arg3.fd = fd;
    snprintf(thread_arg1.thread_name, sizeof(thread_arg1.thread_name), "%s",
             "clone 1");
    snprintf(thread_arg2.thread_name, sizeof(thread_arg2.thread_name), "%s",
             "clone 2");
    snprintf(thread_arg3.thread_name, sizeof(thread_arg3.thread_name), "%s",
             "clone 3");
    pthread_create(&thread1, NULL, thread_func, &thread_arg1);
    pthread_create(&thread2, NULL, thread_func, &thread_arg2);
    pthread_create(&thread3, NULL, thread_func, &thread_arg3);

    for (int i = 0; i <= offset; i++) {
        lseek(fd, i, SEEK_SET);
        if ((sz = read(fd, &bn_buf, sizeof(bn_buf))) < 0) {
            printf("Reading failed at offset %d [%lld]\n", i, sz);
        } else {
            print_BigN_string(bn_buf, fib_str_buf, sizeof(fib_str_buf));
